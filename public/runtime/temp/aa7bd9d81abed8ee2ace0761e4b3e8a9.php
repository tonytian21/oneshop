<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:45:"../oscshop/mobile/view/custom_menu/index.html";i:1505890083;s:38:"../oscshop/admin/view/public/base.html";i:1505885316;}*/ ?>
<!DOCTYPE html>
<html lang="en">
	<head>
...
<div class="row">
	...
	<?php if(empty($menu)): $__FOR_START_605597080__=0;$__FOR_END_605597080__=3;for($i=$__FOR_START_605597080__;$i < $__FOR_END_605597080__;$i+=1){ ?>
	<div class="col-xs-12">	
	...
	<?php } else: if(is_array($menu) || $menu instanceof \think\Collection || $menu instanceof \think\Paginator): $k = 0; $__LIST__ = $menu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$menu): $mod = ($k % 2 );++$k;?>
		<div class="col-xs-12">	
			<div class="table-responsive">
					<table class="table table-striped table-bordered table-hover">  	
						<tr>
							<th class="col-xs-2">第<?php echo $k+1; ?>列菜单类型</th>
							<th class="col-xs-5">第<?php echo $k+1; ?>列菜单名称</th>
							<th class="col-xs-5">第<?php echo $k+1; ?>列菜单内容</th>
							<th></th>
						</tr>
						<tbody id="tbody<?php echo $k; ?>">
						<?php if(empty($menu['sub_button'])): ?>	
						<tr>
							<td>							
								<select class="form-control" name="type[<?php echo $k; ?>][0]" id="select<?php echo $k; ?>">								
									<?php if(is_array($menu_type) || $menu_type instanceof \think\Collection || $menu_type instanceof \think\Paginator): if( count($menu_type)==0 ) : echo "" ;else: foreach($menu_type as $key=>$m): ?>
							...
							<?php if($menu['type'] == 'view'): ?>
								<td>
									<input type="text" name="name[<?php echo $k; ?>][0]" value="<?php echo $menu['name']; ?>" class="form-control">
								</td>
								<td>
									<input type="text" name="content[<?php echo $k; ?>][0]" value="<?php echo $menu['url']; ?>" class="form-control">
								</td>
							<?php endif; ?>
						</tr>
						...
						<?php else: ?>
							<tr>
								...
								<td>
									<input type="text" name="content[<?php echo $k; ?>][0]" value="" class="form-control">
								</td>
							...
							<?php $sb_key=0; if(is_array($menu['sub_button']) || $menu['sub_button'] instanceof \think\Collection || $menu['sub_button'] instanceof \think\Paginator): $i = 0; $__LIST__ = $menu['sub_button'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$m1): $mod = ($i % 2 );++$i;$sb_key+=1; ?>
								<tr id="<?php echo $k; ?>input<?php echo $sb_key; ?>">
									<td>							
										<select class="form-control" name="type[<?php echo $k; ?>][<?php echo $sb_key; ?>]" id="select<?php echo $sb_key; ?>">								
											<?php if(is_array($menu_type) || $menu_type instanceof \think\Collection || $menu_type instanceof \think\Paginator): if( count($menu_type)==0 ) : echo "" ;else: foreach($menu_type as $key=>$m): ?>
									...
									<?php if($m1['type'] == 'view'): ?>
										<td>
											<input type="text" name="name[<?php echo $k; ?>][<?php echo $sb_key; ?>]" value="<?php echo $m1['name']; ?>" class="form-control">
										</td>
										<td>
											<input type="text" name="content[<?php echo $k; ?>][<?php echo $sb_key; ?>]" value="<?php echo $m1['url']; ?>" class="form-control">
										</td>
									<?php else: ?>
										<td>
											<input type="text" name="name[<?php echo $k; ?>][<?php echo $sb_key; ?>]" value="<?php echo $m1['name']; ?>" class="form-control">
										...
										<td>
											<input type="text" name="content[<?php echo $k; ?>][<?php echo $sb_key; ?>]" value="<?php echo (isset($m1['url']) && ($m1['url'] !== '')?$m1['url']:''); ?>" class="form-control">
										</td>
									<?php endif; ?>
									<td><button type="button" onclick="$('#'+'<?php echo $k; ?>'+'input'+'<?php echo $sb_key; ?>').remove();" class="btn btn-danger"><i class="fa fa-trash"></i></button></td>
								</tr>
						...
						<tr>
							<td colspan="3">
								<div class="btn btn-primary " bid='0' id="b<?php echo $k; ?>" onclick="add_child_menu('<?php echo $k; ?>');">添加子菜单</div>
							</td>
						</tr>
					</table>
